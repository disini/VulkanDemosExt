...
#define VULKAN_SAMPLES_BASE_DIR "/media/liusheng/Programfiles/Programs/VulkanSDK/1.2.148.1/ubuntu/samples"
...
VkResult write_ppm(struct sample_info& info, const char* basename) {
    ...
    // Reset the used commandBuffer for next present
    set_image_layout(info, info.buffers[info.current_buffer].image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
            VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT);
...
bool write_png(struct sample_info& info, const char* basename) {
    string filename;
    ...
    // Create a mappable image
    res = vkCreateImage(info.device, &image_create_info, NULL, &mappableImage);
    cout << "utils::write_png() : Create a mappable image" << endl;
    assert(res == VK_SUCCESS);
    ...
    vkGetImageSubresourceLayout(info.device, mappableImage, &subres, &src_layout);

    unsigned char* srcPtr;
    res = vkMapMemory(info.device, mappableMemory, 0, mem_reqs.size, 0, (void**)&srcPtr);
    assert(res == VK_SUCCESS);

    srcPtr += src_layout.offset;
    //ofstream file(filename.c_str(), ios::binary);
    //char fName[1024] = { 0 };
    //char fName[1024] = filename.c_str();
    char* fName = const_cast<char*>(filename.c_str());
//    strcpy(filenameArr, filename.length(), filename.c_str());
    strcpy(filenameArr, filename.c_str());
    png_structp png_ptr;
    png_infop info_ptr;
    png_colorp palette;

    //sprintf(fName, filename.c_str(), )
    ...
    /* 接下来告诉 libpng用fwrite来写入PNG文件，并传给它已按二进制方式打开的FILE* fp */
    ...
    // 分配调色板空间， 常数PNG_MAX_PALETTE_LENGTH 的值是256
    ...
    image = (png_byte*)srcPtr;
    /*const int* row = (const int*)srcPtr;*/


    if (info.height > PNG_UINT_32_MAX / sizeof(png_bytep))
    {
	...
	int swapped;
	int R = 0, G = 0, B = 0, A = 0;
    unsigned char* totalRGBAData = nullptr;
    unsigned char* pRGBAData = nullptr;
    int bytesPerline = info.width * 32;
    int totalPixelBytes = bytesPerline * info.height;
    totalRGBAData = (unsigned char*)malloc(totalPixelBytes);
    ...
    pRGBAData = totalRGBAData;

        //row_pointers[k] = (png_bytep)(image + k * info.width * 4);// wrong color! 	

		if (info.format == VK_FORMAT_B8G8R8A8_UNORM || info.format == VK_FORMAT_B8G8R8A8_SRGB) {
            for (k = 0; k < info.height; k++)
            {
                const int* row = (const int*)srcPtr;
                for (x = 0; x < info.width; x++) {
                    //swapped = (*row & 0xff00ff00) | (*row & 0x000000ff) << 16 | (*row & 0x00ff0000) >> 16;
...
//                    #if __BYTE_ORDER == __LITTLE_ENDIAN
                    ...
                    int transformedColor = R << 24 | G << 16 | B << 8 | A;
                    //row_pointers[k][x] = 
                    pRGBAData[0] = (unsigned char)R;
                    pRGBAData[1] = (unsigned char)G;
                    pRGBAData[2] = (unsigned char)B;
                    pRGBAData[3] = (unsigned char)A;
                    //file.write((char*)&swapped, 3);
                   
                    pRGBAData += 4;
                    row++;
                }
                //row_pointers[k] = (png_bytep)(totalRGBAData + k * info.width * 4);
                //row_pointers[k] = (png_bytep)(totalRGBAData - (k + 1) * info.width * 4);
                row_pointers[k] = (png_bytep)(pRGBAData - info.width * 4);
                srcPtr += src_layout.rowPitch;
            }
		}
		else if (info.format == VK_FORMAT_R8G8B8A8_UNORM) {
            for (k = 0; k < info.height; k++)
    ...
    if(totalRGBAData != nullptr)
    {
        free(totalRGBAData);
        totalRGBAData = nullptr;
    }
...
//	printf("write_png() : %s", filename);
    printf("write_png() : %s", filenameArr);
